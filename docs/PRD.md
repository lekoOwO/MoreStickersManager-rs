...
| Web UI | Implemented | Desktop/mobile shell, i18n, theme, separated local login/local registration/SSO dialogs, PAT management with an isolated create-PAT flow and role-filtered scope discovery, OIDC login-start/callback controls, pack CRUD/import, provider import planning/job/config controls, product metadata create/list, product membership add/remove controls, tenant member/settings/user-status/role-template/OIDC-provider administration, export target/job/recovery UI, publication history, Telegram reconciliation controls, and portable user migration controls exist. Embedded production builds now default to the same-origin API when no build-time `VITE_MSM_API_BASE_URL` is provided, while plain Vite development without an API base remains mock-preview capable. `VITE_MSM_PAT` is development-only and is ignored by production builds. |
...
| CI/release | Implemented | CI, Docker publish, prerelease, release workflows, npm workspace flow for local/CI Web checks, Bun-based Docker Web build, Dockerfile, and dev manager exist. |
