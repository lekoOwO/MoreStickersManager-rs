...
- Development environment manager slice: added a Node.js dev manager for starting/stopping API and Web dev services, status checks, environment profile switching, tracked env examples, npm/pnpm script shortcuts, Windows-safe process spawning, local runtime directory creation, and handoff/user documentation.

Current task:
- Return to post-publication mappings after the dev manager slice: initial publication populates mappings; next step is refreshing mappings after reconciliation mutation execution and using them to build fetched remote state automatically.
