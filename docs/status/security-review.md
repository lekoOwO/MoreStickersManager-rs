...
| MCP transport | Acceptable for current contract | `/mcp` is stateless JSON-RPC over POST, sets no-store, and rejects SSE GET negotiation with a structured unsupported-session response. Tool calls use Bearer PAT enforcement, and the current contract intentionally does not implement stateful MCP sessions. | Revisit only if future MCP clients require SSE/resumability semantics. |
