...
- OIDC provider configuration storage now exists for per-tenant issuer, client,
  scope, enabled-state, and registration-policy settings. API OIDC start/state and callback completion now exist with authorization-code exchange, discovery, signed ID-token validation, and userinfo fallback. CLI and MCP provider list/upsert/delete commands/tools now exist; Web admin/client surfaces are next.
