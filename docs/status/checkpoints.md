...
## 2026-05-10 PostgreSQL Tag Repository Contract

- Added backend-aware SQLite/PostgreSQL SQL paths for tag create, list, and find operations.
...
## 2026-05-10 PostgreSQL Metadata Membership Repository Contract

- Added backend-aware SQLite/PostgreSQL SQL paths for folder-pack, pack-tag, and subscription-group-pack membership add/list operations.
...
## 2026-05-10 PostgreSQL Local Credential Repository Contract

- Added backend-aware SQLite/PostgreSQL SQL paths for local-credential create/read/verify operations.
...
- Added MSM-side CORS configuration with `MSM_CORS_ALLOWED_ORIGINS`, allowing configured Discord/Equicord browser origins to fetch MSM API/subscription/asset routes directly.
- Documented the upstream plugin adapter contract in `docs/integrations/equicord-morestickers-msm-adapter.md`.
- Prepared `docs/integrations/patches/equicord-morestickers-msm-adapter.patch` against upstream Equicord commit `3a45528`, covering MSM subscription UI, direct MSM fetches, dynamic pack-set sync, startup refresh, and protected asset blob-cache rendering/sending support.
- Added `docs/integrations/equicord-morestickers-msm-adapter-patch.md` with patch scope, apply command, and verification limits.
- Updated PRD/current status/implementation matrix to track remaining upstream apply/build/runtime verification.
- Verification: `cargo test -p msm-api --locked`; `cargo test -p msm-app --locked`; `cargo clippy -p msm-api -p msm-app --all-targets --locked -- -D warnings`; `cargo fmt --all -- --check`; `git diff --check`; `git -C tmp/Equicord-moreStickers diff --check`; `git diff --check -- docs/integrations/patches/equicord-morestickers-msm-adapter.patch`; `git -C tmp/Equicord-patch-check apply --check <patch>` against clean Equicord `3a45528`. Rust verification used `TMP`/`TEMP=D:\Temp`, `CARGO_INCREMENTAL=0`, `CARGO_BUILD_JOBS=1`, and `CARGO_TARGET_DIR=target\msm-equicord-cors`. Equicord TypeScript/runtime verification remains pending because the sparse clone has no installed dependencies and the current shell has no global `tsc`.
