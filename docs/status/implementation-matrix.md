...
| Export jobs | P29-P30 | Partially implemented: storage tables/repositories, protected API/OpenAPI routes, prepared media cache writes, process-backed ffmpeg executor, target bootstrap config, and an optional app worker loop exist for targets, queued jobs, job status, and job events. Telegram publication, retries, and Web/CLI/MCP surfaces are not implemented. |
...
P25 should continue from `docs/superpowers/plans/2026-05-06-msm-telegram-export-pipeline.md` Task 10 and add CLI/MCP parity for export targets and jobs.
