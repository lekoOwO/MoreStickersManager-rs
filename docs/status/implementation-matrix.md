...
| Remote target sync/update/delete | P33+ | Telegram reconciliation policies are defined in `msm-exporters` for create-only, append-missing, and mirror operations. Fetching remote state and executing update/delete through `msm-telegram` and the worker is still pending. |

## Current Next Phase

Continue by wiring remote Telegram state fetch/update/delete execution behind safe worker options.
