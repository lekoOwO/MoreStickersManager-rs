...
| Web pack management | Implemented | Dashboard supports shell-selected pack list, rename, visibility edit, delete, pasted `.stickerpack` import dialog, export target setup, export job queue/status views, Telegram publication history links, and Telegram reconciliation controls for selected packs. |
...
| Folder/tag management | Next phase | Database tables, storage repository primitives, protected API routes, OpenAPI schemas, CLI commands, and MCP tools exist for folder create/list and tag create/list. Web management surfaces are not implemented. |
| Subscription groups | Next phase | Domain/storage foundations, repository primitives, protected API routes, OpenAPI schemas, CLI commands, and MCP tools exist for subscription group create/list. Pack membership management, subscription links, Web surfaces, and moreStickers auto-update integration remain later work. |
...
Continue with Web surfaces for folder, tag, and subscription group metadata, then expand pack access-management APIs beyond basic visibility.
