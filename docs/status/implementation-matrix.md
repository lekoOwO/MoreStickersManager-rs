...
| Remote target sync/update/delete | P33+ | Telegram reconciliation policies are defined in `msm-exporters` for create-only, append-missing, and mirror operations. `msm-telegram` can execute title/add/replace/delete mutation sequences, and worker dry-run jobs can summarize planned operations from supplied remote state. Fetching remote state and executing non-dry-run mutations is still pending. |

## Current Next Phase

Continue by adding guarded non-dry-run Telegram reconciliation execution and remote state retrieval.
