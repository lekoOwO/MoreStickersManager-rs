...
| Remote target sync/update/delete | P33+ | Telegram reconciliation policies are defined in `msm-exporters` for create-only, append-missing, and mirror operations. `msm-telegram` can execute title/add/replace/delete mutation sequences and fetch remote sticker set metadata, worker dry-run jobs can summarize planned operations from supplied remote state, append-missing non-dry-run jobs can execute supplied-state mutations with `executeReconciliation:true`, and mirror replace/delete also requires `allowDestructiveReconciliation:true`. Automatic fetched-state reconciliation awaits persisted per-sticker source ID mapping. |

## Current Next Phase

Continue by designing and persisting Telegram per-sticker source ID mapping.
